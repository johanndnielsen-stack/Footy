...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { code, hg, ag, playerName, avg } = body;
    ...
    // Refresh the syncing player's squad rating so the standings AVG reflects
    // their current (upgraded) team, not the value stored when they joined.
    if (playerName && avg != null && data.teams) {
      const me = data.teams.find(t => t.name === playerName);
      if (me) me.avg = avg;
    }
