exports.handler = async (event) => {
  ...
  const { league, lang, topic, recentQs } = body;
  if (!league || !lang) {
    return { statusCode: 400, body: "Missing league or lang" };
  }

  const topicLine = topic
    ? `Focus this round on the sub-topic: "${topic}".`
    : "";
  const avoidLine =
    Array.isArray(recentQs) && recentQs.length > 0
...
${topicLine}
${avoidLine}
Rules: questions must be factual and verifiable. Cover history, records, players, clubs, rules, competitions. Make each question feel distinct — vary the subject, era, and angle.
